...
#                 types and tags each row as commercial. Reads the island column
#                 derived from area_id during cleaning.
#
...
#*****************************************************************
    ...
    def computeSummary(self, commercial, noncommercial):
        ...
        for r in commercial + noncommercial:
            i = r['island']
            if i:
                island_totals[i] = island_totals.get(i, 0) + r['exchange_value']
        top_island = max(island_totals, key=island_totals.get) if island_totals else '—'
        top_island_val = island_totals.get(top_island, 0)
        ...
        return {
            ...
            'top_island': top_island,
            'top_island_val': top_island_val,
...
#*****************************************************************
    ...
    def buildHtml(self, commercial, noncommercial, summary):
        ...
        top_island_val_display = self.fmtMoney(s['top_island_val'])
...
<div class="main-bg"><div class="container">
    <div class="stats-row">
        ...
        <div class="stat-card"><div class="stat-label">Top Island</div><div class="stat-value" style="font-size:1.2rem">{s['top_island']}</div><div class="stat-sub">{top_island_val_display}</div></div>
    ...
    <div class="controls-panel"><div class="controls-title">&#9881; Filters &amp; Controls</div><div class="controls-grid">
        ...
        <div class="ctrl-group"><label>Island</label><select id="fRegion"><option value="all">All Islands</option></select></div>
    ...
    <div class="charts-grid">
        ...
        <div class="chart-card"><div class="chart-header"><div><div class="chart-title">By Island</div><div class="chart-subtitle">Geographic distribution</div></div></div><div class="chart-wrap"><canvas id="cIsland"></canvas></div></div>
        <div class="chart-card"><div class="chart-header"><div><div class="chart-title">By County</div><div class="chart-subtitle">Administrative grouping</div></div></div><div class="chart-wrap"><canvas id="cCounty"></canvas></div></div>
...
function filtered(){{let data=[];if(S.source!=='noncommercial')data=data.concat(DATA_COMMERCIAL);if(S.source!=='commercial')data=data.concat(DATA_NONCOMMERCIAL);return data.filter(d=>d.year>=S.yearMin&&d.year<=S.yearMax&&(S.region==='all'||d.island===S.region)&&(S.species==='all'||d.species_group===S.species)&&(S.ecosystem==='all'||d.ecosystem_type===S.ecosystem));}}
function populateFilters(){{const all=[...DATA_COMMERCIAL,...DATA_NONCOMMERCIAL];const islands=new Set(),species=new Set(),ecos=new Set();all.forEach(d=>{{if(d.island)islands.add(d.island);if(d.species_group)species.add(d.species_group);if(d.ecosystem_type)ecos.add(d.ecosystem_type);}});fillSelect('fRegion',islands,'All Islands');fillSelect('fSpecies',species,'All Species');fillSelect('fEco',ecos,'All Ecosystems');}}
