...
def main():
    ...
    print(f" Timestamped files saved to: data/cleaned/")

    # -------------------------------------------------
    # COPY TO DASHBOARD PUBLIC FOLDER (stable filenames)
    # -------------------------------------------------
    import shutil

    dashboard_dir = Path(__file__).parent.parent / 'capstone-dashboard' / 'public' / 'fisheriesdata'

    if dashboard_dir.exists():
        comm_src = pipeline.results['commercial']['output_file']
        noncomm_src = pipeline.results['non_commercial']['output_file']

        shutil.copy2(comm_src, dashboard_dir / 'cleaned_commercial.csv')
        shutil.copy2(noncomm_src, dashboard_dir / 'cleaned_noncommercial.csv')

        print(f" Dashboard data updated: {dashboard_dir}")
    else:
        print(f" Dashboard directory not found at {dashboard_dir}")
        print(f"  Copy cleaned CSVs manually to your dashboard's public/fisheriesdata/")
